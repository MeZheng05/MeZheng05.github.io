...
str_detect(sales$released, "[:graph:]US[:graph:][:graph:][1234567890][1234567890][:graph:]|[:graph:]UK[:graph:][:graph:][1234567890][1234567890][:graph:]")
#removes them
sales$released <- str_replace(sales$released, "[:graph:]US[:graph:][:graph:][1234567890][1234567890][:graph:]|[:graph:]UK[:graph:][:graph:][1234567890][1234567890][:graph:]", "")
...
ts_lyrics1$line[str_detect(ts_lyrics1$line, "[hH]ello")]
...
ts_lyrics1$line[str_detect(ts_lyrics1$line, "[gG]oodbye")]
...
b_lyrics$line[str_detect(b_lyrics$line, "[hH]ello")]
sum(str_count(b_lyrics$line, "[hH]ello"))

#How many lines contain Goodbye?
b_lyrics$line[str_detect(b_lyrics$line, "[Gg]oodbye")]
sum(str_count(b_lyrics$line, "[Gg]oodbye"))
```

There are 162 lines for hello and 12 lines that contains goodbye. 
...
data <- sales %>% mutate(Album = title) %>% filter(country == "US") %>% select(-title) %>% inner_join(avg_score, by = "Album")
head(data)
