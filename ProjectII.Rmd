...
rainfall$year <- format(as.Date(rainfall$date, "%d-%m-%Y"), "%Y")
```
...
```{r}
new_rainfall <- inner_join(rainfall, temperature, by = c("date", "city_name"))
nrow(new_rainfall)
ncol(new_rainfall)
```
...
new_rainfall$year <- as.numeric(new_rainfall$year)
rainfall_2014 <- new_rainfall %>% filter(year >= 2014)
...
* The Code of My Initial Function

```{r}
myplot <- function(city, year){
  if(!city %in% rainfall$city_name){
    stop("City NOT in the list!", call. = FALSE)
  } #if the input city does not exist in the dataset, stop the function and return the error
  else if(!year %in% rainfall$year){
    stop("Year NOT in the list!", call. = FALSE)
  } #if the input year does not exist in the dataset, stop the function and return the error
  else{
    data = new_rainfall %>% filter(city_name == city, year == year) #filter the dataset with specified city name and year
    data %>%
    ...
    geom_histogram(aes(fill = city), bins = 10) +
      ...
      theme_bw() + 
      annotate("text", x = 5, y = 600, label = c("Year")) +
      annotate("text", x = 5.6, y = 600, label = c(year)) #for a given year, label it on the plot
  }
}
...
This histogram shows the distribution of log of rainfall amount in Perth city, 1995. 
...
PERTH is in our records, but the earliest rainfall records is from 1920, which does not include years before that. Entering 1200 gives the error "Year NOT in the list!". 
...
City PERU is not in our records. Entering PERU gives the error "City NOT in the list". 
...
myplot2 <- function(city, year = year){
  if(!toupper(city) %in% rainfall$city_name){
    stop("City NOT in the list!", call. = FALSE)
  } #if the input city does not exist in the dataset, stop the function and return the error
  else if(!year %in% rainfall$year){
    stop("Year NOT in the list!", call. = FALSE)
    ...
    data = new_rainfall %>% filter(city_name == toupper(city), year == year) #filter the dataset with specified city name and year
    data %>%
    ggplot(aes(log(rainfall))) + 
    geom_histogram(aes(fill = city), bins = 10) +
      labs(title = "Histogram of Annual Rainfall within a City", caption = "Meilin Zheng", x = "log of Rainfall Amount", y = "Count") + 
...
rainfall4 <- new_rainfall %>% filter(year >= 2014)
